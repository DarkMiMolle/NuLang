...
// token.CHAR, token.STRING) or token.COMMENT, the literal string
...
scanAgain:
	...
	case isLetter(ch):
		...
		if len(lit) > 1 {
			...
			switch tok {
			case token.IDENT, token.BREAK, token.CONTINUE, token.RETURN, token.DO, token.THEN, token.THROW, token.EXEC:
				insertSemi = true
			}
		} else {
			insertSemi = true
	...
	default: // scan special char
		...
		case '=':
			tok = s.switch3(token.ASSIGN, token.EQL, '>', token.ARROW)
		case '!':
