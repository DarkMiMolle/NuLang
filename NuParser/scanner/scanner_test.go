...
import (
	...
	"github.com/DarkMiMolle/NuLang/NuParser/scanner/token"
)
...
var tokens = [...]elt{
	...
	{token.EXEC, "exec", keyword},
	{token.EXPLICIT, "explicit", keyword},
	{token.EXTENSION, "extension", keyword},
	...
	{token.IF, "if", keyword},
	{token.IMPLICIT, "implicit", keyword},
	...
	{token.OVERRIDE, "override", keyword},
	{token.PACKAGE, "package", keyword},
	{token.RETURN, "return", keyword},

	{token.SELECT, "select", keyword},
	{token.STATIC, "static", keyword},
	{token.STRUCT, "struct", keyword},
	{token.SWITCH, "switch", keyword},
	{token.RUN, "run", keyword},

	{token.THEN, "then", keyword},
	{token.THROW, "throw", keyword},
	{token.TO, "to", keyword},
	{token.TRY, "try", keyword},
	{token.TYPE, "type", keyword},
	{token.TYPEOF, "typeof", keyword},
...
func TestScan(t *testing.T) {
	...
	for {
		...
		if tok != e.tok {
			t.Errorf("bad token for %q: got %s, expected %s (%s)", lit, tok, e.tok, e.lit)
		}
...
func TestInit(t *testing.T) {
	...
	_, tok, _ = s.Scan() // go
	if tok != token.RUN {
		t.Errorf("bad token: got %s, expected %s", tok, token.RUN)
	}
